...
class StableDiffusion(nn.Module):
    def __init__(self, device, fp16, vram_O, sd_version='2.1', hf_key=None, t_range=[0.02, 0.98]):
        ...
        #Hao
        #pipe = StableDiffusionPipeline.from_pretrained( "/home/haozhang/.cache/huggingface/hub/models--stabilityai--stable-diffusion-2-1-base/snapshots/88bb1a46821197d1ac0cb54d1d09fb6e70b171bc", torch_dtype=precision_t)
        #end
    ...
    def train_step(self, text_embeddings, pred_rgb, guidance_scale=100, as_latent=False, grad_clip=None):
        ...
        if as_latent:
            ...
            pred_rgb_512 = pred_rgb_512.type(torch.cuda.HalfTensor)
        ...
        ####################################
        # imgs = self.decode_latents(noise_pred) # [1, 3, 512, 512]

        # # Img to Numpy
        # imgs = imgs.detach().cpu().permute(0, 2, 3, 1).numpy()
        # imgs = (imgs * 255).round().astype('uint8')
        #np.save("./noise_glasses/"+str(self.counter)+".npy",imgs)
        ...
        grad_detach = (grad_detach**2).sum()
        # import pdb
        # pdb.set_trace()
        return loss,grad_detach
