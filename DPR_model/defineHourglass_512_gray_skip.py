...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.inplanes != self.outplanes:
            ...
            #print("the shape of the out, if condition",out.shape)
        else:
            out = out + x 
            #print("the shape of the out, else condition",out.shape)
...
class lightingNet(nn.Module):
    ...
    def forward(self, innerFeat, target_light, count, skip_count):
        #print("innerFeat shape",id(innerFeat))
        ...
        #print(id(innerFeat))
        #innerFeat[:,0:self.ncInput,:,:] = upFeat    ######### this is the in-place operation
        #print("before", id(innerFeat))
        innerFeat = torch.cat((upFeat,innerFeat[:,self.ncInput:,:,:]),1)
        #print("after", id(innerFeat))
        #print(id(innerFeat))
...
class HourglassNet(nn.Module):
    ...
    def forward(self, x, target_light, skip_count):   # skip_count = 0
